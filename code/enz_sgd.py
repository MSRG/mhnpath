...
sys.stdout = open('enz_sgd_metrics.txt', 'w')
...
# Initialize the sgdmodel with the Matern kernel
model = SGDRegressor()
...
for i in range(num_batches):
    ...
    try:
        model.partial_fit(x_flat, Y_batch)
        print("fitted: ", i + 1)
    except:
...
for start_idx in range(num_batches * batch_size, len(products), batch_size):
    ...
    del test_products, mapped_test_str, mapped_test, product_encoded_test, X_test, Y_test, X_test_flat, y_pred, y_p
