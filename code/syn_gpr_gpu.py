...
sys.stdout = open('syn_gpr_gpu_15_metrics.txt', 'w')

# Path to the CSV file
path = 'syn_shuffled.csv'

mapped_path = 'syn_shuffled_mapped.csv'
...
length_scale = 1.0
nu = 1.5
matern_kernel = tfp.math.psd_kernels.MaternOneHalf(length_scale=length_scale)

# Initialize the Gaussian Process model with the Matern kernel
model = tfp.distributions.GaussianProcess(kernel=matern_kernel, index_points=products, observation_noise_variance=1e-5)
...
for i in range(num_batches):
    ...
    model = model.experimental_fit(x_tensor, y_tensor, num_kernel_train_points=10, num_monte_carlo_samples=10)

    # Evaluate the model
    loss = model.log_prob(x_tensor, y_tensor)

    print(f'Batch {i + 1} - Loss: {loss}')

    del mapped_batch, mapped_batch_str, product_encoded_batch, X_batch, Y_batch, x_flat, batch_products, x_tensor, y_tensor

joblib.dump(model, 'syn_gpu_gaussian_process_model_15.joblib')
