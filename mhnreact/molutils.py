...
def getFingerprint(smiles, fp_size=4096, radius=2, is_smarts=False, which='morgan', sanitize=True):
    ...
    if mol is None:
        ...
        logger.warning(msg)
        #warnings.warn(msg)
        return np.zeros(fp_size).astype(bool)
    ...
    if isinstance(list_of_smarts, list):
        if isinstance(list_of_smarts[0], list):
            ...
            fps = [[getTemplateFingerprint(smarts, fp_size=fp_size) for smarts in sample]
                   + [np.zeros(fp_size, dtype=bool)] * (pad - len(sample))  # zero padding
...
def smarts2rdkfp(smart, fp_size=2048):
    ...
    if mol is None: return np.zeros(fp_size).astype(bool)
    return AllChem.RDKFingerprint(mol)
    # fp = np.asarray(fp).astype(bool) # takes ages =/
...
def getTemplateFingerprint(smarts, fp_size=4096):
    ...
    return np.array(list(AllChem.CreateStructuralFingerprintForReaction(rxn, )), dtype=bool)
