...
if __name__ == '__main__':
    ...
    if args.model_type == 'fortunato' or args.pretrain_epochs>1:
        ...
        for epoch in range(args.pretrain_epochs):
            ...
            clf.evaluate(X_fp['valid'], X_fp['valid'], y_appl['valid'], 
                                  split='pretrain_valid', is_smiles=False, only_loss=True, 
                                  bs=args.batch_size,wandb=wandb)
            # appl_acc = ((y_appl['valid'].toarray()) == (y_pred>0.5)).mean()
            print(f'{epoch:2.0f} -- train_loss: {clf.hist["loss"][-1]:1.3f}, loss_valid: {clf.hist["loss_pretrain_valid"][-1]:1.3f}')
    ...
    output_file_path = os.path.join('data', 'model', f'{fn_postfix}_training_data.txt')
    with open(output_file_path, 'w') as f:
        f.write('Cumulative Examples\tLoss\n')
        for examples, loss in zip(clf.hist['cumulative_examples'], clf.hist['loss']):
            f.write(f'{examples}\t{loss}\n')
